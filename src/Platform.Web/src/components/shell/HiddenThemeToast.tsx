 ...
 * to undo it. Announced as a live region for the same reason. Also confirms the sound toggle.
 *
...
export function HiddenThemeToast() {
  ...
  const lastChange = useHiddenThemeStore((s) => s.lastChange);
    ...
    body = sound ? (
      <>
        ...
        <span style={{ color: 'var(--text-secondary)' }}>
          {info ? `${info.name} ambience, synthesised live.` : 'Pick a theme to hear it.'}
        </span>
        <Hint keys={info ? 't s' : 't t'} label={info ? 'to mute' : 'to pick one'} />
      </>
    ) : (
      <>
        <span className="font-semibold">Sound off</span>
        <Hint keys="t s" label="to bring it back" />
      </>
    );
  } else if (info) {
    body = (
      <>
        ...
        <Hint keys="t s" label={sound ? 'to mute' : 'for sound'} />
      </>
    );
  } else {
    body = (
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className="hidden-theme-toast fixed bottom-6 left-1/2 -translate-x-1/2 z-[1300] flex flex-wrap items-center justify-center gap-x-3 gap-y-1 max-w-[92vw] px-4 py-2.5 rounded-full text-[13px] whitespace-nowrap pointer-events-none"
      style={{
