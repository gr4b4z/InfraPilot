...
| `title` | string | no | Human-readable title (e.g. work-item summary, PR title). When supplied for a `work-item` reference, the server uses it directly and skips the Jira lookup. Producers that derive work items from commit messages should put the **commit subject line** here — it usually describes the change better than the tracker's summary — and move the tracker's own summary to `subTitle`. |
| `subTitle` | string | no | Secondary display line under `title`. Meaningful on `work-item` references: when `title` carries the commit subject, this carries the tracker's own summary (e.g. the Jira ticket title) so a reviewer can still recognise the ticket by its Jira name. Omit when it would repeat `title`. |
